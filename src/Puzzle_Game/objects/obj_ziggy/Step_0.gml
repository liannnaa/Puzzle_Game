if(room == rm_store){
	// move up
	if(keyboard_check(vk_up) && !place_meeting(x, y + walk_speed+5, obj_wall_h)){
		image_speed = .25
		y -= walk_speed
		return
	}
	...
	// move down
	if(keyboard_check(vk_down) && !place_meeting(x, y - walk_speed-5, obj_wall_h)){
		image_speed = .25
		y += walk_speed
		return
	}
...
if(keyboard_check(vk_down) && place_meeting(x, y, obj_ladder)){
	image_speed = 0
	image_index = 1
	y += walk_speed
	return
...
if (!player_falling && keyboard_check(vk_up))
{
	player_jumping = true;
...
if(player_jumping && keyboard_check_released(vk_up))
{
